...
CREATE TABLE IF NOT EXISTS public.document_logic_conditions (
  ...
  field bigint REFERENCES public.inputs(id) ON DELETE SET NULL,
  operator text,
  value text,
  value_type text DEFAULT 'value',
  value_expression text,
  value_field bigint REFERENCES public.inputs(id) ON DELETE SET NULL,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
COMMENT ON TABLE public.document_type_ai_condition IS 'AI conditions for document types — prompts that evaluate uploaded docs';
...
CREATE TABLE IF NOT EXISTS public.document_type_ai_input (
  ...
  input_id bigint REFERENCES public.inputs(id) ON DELETE SET NULL,
  ai_prompt text,
  created_at timestamptz NOT NULL DEFAULT now()
);
