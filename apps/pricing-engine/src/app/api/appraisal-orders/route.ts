...
import { getOrgUuidFromClerkId, getUserRoleInOrg, isPrivilegedRole, authForApiRoute } from "@/lib/orgs"
...
export async function GET() {
  try {
    ...
    try {
      ({ userId, orgId } = await authForApiRoute("appraisals", "read"))
    } catch (e: unknown) {
...
export async function POST(req: NextRequest) {
  try {
    let userId: string, orgId: string
    try {
      ({ userId, orgId } = await authForApiRoute("appraisals", "write"))
    } catch (e: unknown) {
      const status = (e as { status?: number }).status ?? 401
      return NextResponse.json({ error: (e as Error).message }, { status })
    }
