  ...
  try {
    ...
    if (ownerEntityIds.length > 0) {
      ...
      ownerEntityEinMap = Object.fromEntries(
        (entitiesData ?? []).map((e) => [
          e.id as string,
          {
            display_id: (e.display_id as string) ?? null,
            entity_name: (e.entity_name as string) ?? null,
    ...
    if (borrowerIds.length > 0) {
      ...
      borrowerMap = Object.fromEntries(
        (borrowers ?? []).map((b) => [
          b.id as string,
          (b.display_id as string) ?? "",
        ])
      )
    }

    const enrichedOwners = (owners ?? []).map((o: any) => {
      const ent = o?.entity_owner_id
        ? ownerEntityEinMap[o.entity_owner_id]
        : null
      ...
      return {
        ...
        borrower_display_id: o?.borrower_id
          ? (borrowerMap[o.borrower_id] ?? null)
          : null,
      }
    })
    ...
    return NextResponse.json({
      ...
      owned_by_entities: (owners ?? [])
        .map((o) => o.entity_owner_id)
        .filter(Boolean),
