...
import { ensureGoogleMaps } from "@repo/lib/google-maps"
...
async function saveFileWithPrompt(file: File): Promise<void> {
  try {
    ...
    if (
      name === "AbortError" ||
      /cancel/i.test(String((e as any)?.message ?? ""))
    ) {
...
function programDisplayName(
  ...
  if (!p) return "Program"
  return isBroker
    ? (p.external_name ?? "Program")
    : (p.internal_name ?? p.external_name ?? "Program")
}
...
function ScaledTermSheetPreview({
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      el.removeEventListener(
        "wheel",
        onWheel as EventListener as unknown as (e: WheelEvent) => void
      )
  ...
  useEffect(() => {
    if (readOnly) return
    const node = (pageRef as React.RefObject<HTMLDivElement> | undefined)
      ?.current
    if (!node) return
    ...
    >
      ...
      >
        ...
        >
          {(() => {
            const lt = String(
              forceLoanType ?? sheetProps?.loan_type ?? ""
            ).toLowerCase()
      ...
      <div className="pointer-events-auto fixed right-6 bottom-4 z-50">
        <div className="flex items-center gap-2">
          <button
            type="button"
            className="rounded-sm border bg-white px-2 py-1 text-xs text-black shadow-sm hover:bg-neutral-50 dark:bg-white dark:text-black"
            onClick={() => setZoom((z) => Math.max(0.25, z * 0.8))}
            aria-label="Zoom out"
          >
            -
          </button>
          <div className="min-w-14 rounded-sm border bg-white px-2 py-1 text-center text-[11px] text-black shadow-sm dark:bg-white dark:text-black">
...
export default function PricingEnginePage() {
  ...
  async function waitForSelfMemberId(
    maxWaitMs = 45000,
    intervalMs = 400
  ): Promise<string> {
  ...
  const isBroker =
    orgRole === "org:broker" || orgRole === "broker" || isBrokerMember
  // Entity name autocomplete state for Borrower Name input
  const [entityQuery, setEntityQuery] = useState("")
  const [entitySuggestions, setEntitySuggestions] = useState<
    Array<{ id: string; name: string; display: string }>
  >([])
  ...
  const [showGuarantorSuggestions, setShowGuarantorSuggestions] =
    useState(false)
  ...
  const [scenariosList, setScenariosList] = useState<
    { id: string; name?: string; primary?: boolean; created_at?: string }[]
  >([])
    ...
    try {
      const saved =
        typeof window !== "undefined"
          ? window.localStorage.getItem("pricing.mobileView")
          : null
  ...
  useEffect(() => {
    try {
      if (typeof window !== "undefined")
        window.localStorage.setItem("pricing.mobileView", mobileView)
    } catch {
      // ignore
    }
  }, [mobileView])
    ...
    ;(async () => {
      try {
        const res = await fetch(
          `/api/applicants/entities?q=${encodeURIComponent(q)}`,
        ...
        const opts =
          (j.entities ?? []).slice(0, 20).map((e) => ({
            ...
            display:
              `${(e.display_id ?? "") as string} ${(e.entity_name ?? "") as string}`.trim(),
          })) ?? []
        ...
        const opts =
          (j.borrowers ?? []).slice(0, 20).map((b) => ({
            ...
            name: [b.first_name ?? "", b.last_name ?? ""]
              ...
              .trim(),
            display:
              `${(b.display_id ?? "") as string} ${[b.first_name ?? "", b.last_name ?? ""].filter(Boolean).join(" ").trim()}`.trim(),
          })) ?? []
  ...
  const [floodEffective, setFloodEffective] = useState<Date | undefined>(
    undefined
  )
  // Calendar visible months (controlled so arrows work and typing syncs view)
  const [closingCalMonth, setClosingCalMonth] = useState<Date | undefined>(
    undefined
  )
  ...
  const [floodCalMonth, setFloodCalMonth] = useState<Date | undefined>(
    undefined
  )
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const res = await fetch(
          `/api/pricing/programs?loanType=${encodeURIComponent(loanType)}&_=${encodeURIComponent(antiCache)}`,
          {
            method: "GET",
            cache: "no-store",
            headers: {
              ...
              Pragma: "no-cache",
              "X-Client-Request-Id": antiCache,
            },
          }
        )
        if (!res.ok) return
        const pj = (await res.json().catch(() => ({}))) as {
          programs?: Array<{
            id?: string
  ...
  const [streetViewStatus, setStreetViewStatus] =
    useState<google.maps.StreetViewStatus | null>(null)
  const streetViewPanoRef = useRef<HTMLDivElement | null>(null)
  const streetViewPanoInstanceRef =
  ...
  const [warrantability, setWarrantability] = useState<string | undefined>(
    undefined
  ) // Condo only
  const [strValue, setStrValue] = useState<string | undefined>(undefined) // DSCR only
  const [decliningMarket, setDecliningMarket] = useState<string | undefined>(
    undefined
  ) // DSCR only
  const [rural, setRural] = useState<string | undefined>(undefined) // Yes/No
  const [loanStructureType, setLoanStructureType] = useState<
    string | undefined
  >(undefined) // DSCR
  ...
  const [guarantorTags, setGuarantorTags] = useState<
  ...
  const [glaExpansion, setGlaExpansion] = useState<string | undefined>(
    undefined
  ) // Bridge rehab
  ...
  const [programPlaceholders, setProgramPlaceholders] = useState<
    Array<{ id?: string; internal_name?: string; external_name?: string }>
  >([])
  ...
  const [currentLoanId, setCurrentLoanId] = useState<string | undefined>(
    undefined
  )
  const [selectedMainRow, setSelectedMainRow] = useState<SelectedRow | null>(
    null
  )
  ...
  useEffect(() => {
    ...
    setIfUnsetString(
      borrowerType,
      (v) => setBorrowerType(v),
      DEFAULTS.borrowerType
    )
    setIfUnsetString(fthb, (v) => setFthb(v), DEFAULTS.fthb)
    setIfUnsetString(
      citizenship,
      (v) => setCitizenship(v),
      DEFAULTS.citizenship
    )
    setIfUnsetString(
      mortgageDebtValue,
      (v) => setMortgageDebtValue(v),
      DEFAULTS.mortgageDebtValue
    )
    setIfUnsetString(rural, (v) => setRural(v), DEFAULTS.rural)
    setIfUnsetString(strValue, (v) => setStrValue(v), DEFAULTS.strValue)
    setIfUnsetString(
      decliningMarket,
      (v) => setDecliningMarket(v),
      DEFAULTS.decliningMarket
    )
    setIfUnsetString(
      annualFlood,
      (v) => setAnnualFlood(v),
      DEFAULTS.annualFlood
    )
    ...
    setIfUnsetDate(
      acquisitionDate,
      (v) => setAcquisitionDate(v),
      DEFAULTS.acquisitionDate
    )
    setIfUnsetString(
      loanStructureType,
      (v) => setLoanStructureType(v),
      DEFAULTS.loanStructureType
    )
    setIfUnsetString(ppp, (v) => setPpp(v), DEFAULTS.ppp)
    setIfUnsetString(
      borrowerName,
      (v) => setBorrowerName(v),
      DEFAULTS.borrowerName
    )
    // guarantorTags is now an array, no default string needed
    setIfUnsetString(
      uwException,
      (v) => setUwException(v),
      DEFAULTS.uwException
    )
    setIfUnsetString(section8, (v) => setSection8(v), DEFAULTS.section8)
    setIfUnsetString(
      glaExpansion,
      (v) => setGlaExpansion(v),
      DEFAULTS.glaExpansion
    )
    setIfUnsetString(
      changeOfUse,
      (v) => setChangeOfUse(v),
      DEFAULTS.changeOfUse
    )
    setIfUnsetDate(
      hoiEffective,
      (v) => setHoiEffective(v),
      DEFAULTS.hoiEffective
    )
    setIfUnsetDate(
      floodEffective,
      (v) => setFloodEffective(v),
      DEFAULTS.floodEffective
    )
  ...
  useEffect(() => {
    ...
    if (!touched.taxEscrowMonths) {
      setTaxEscrowMonths((v) =>
        v === "" || v === "0" || v === "3" ? currentDefault : v
      )
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loanType])
  ...
  const isPurchase =
    transactionType === "purchase" || transactionType === "delayed-purchase"
  const isRefi = transactionType === "co-refi" || transactionType === "rt-refi"
  const isFicoRequired =
    (isDscr || isBridge) && (citizenship === "us" || citizenship === "pr")
  ...
  const canUseStreetView =
    gmapsReady && !effectiveMapsError && !!mapsCenter && !mapsLoading
  const gmaps =
  ...
  useEffect(() => {
    ...
    geocoder.geocode(
      { address: fullAddress },
      (results: Array<any> | null, status: string) => {
  ...
  useEffect(() => {
    ...
    setStreetViewStatus(null)
    svc.getPanorama(
      { location: mapsCenter, radius: 50 },
      (data: any, status: google.maps.StreetViewStatus) => {
        // #region agent log
        fetch(
          "http://127.0.0.1:7246/ingest/129b7388-6ef0-4f6c-b8cd-48b22b6394cf",
          {
            method: "POST",
  ...
  async function handleSendToReApi(e: React.MouseEvent) {
    ...
    try {
      ...
      const body = JSON.stringify(
        toYesNoDeepGlobal(payload) as Record<string, unknown>
      )
      const headers = {
      ...
      if (!anySuccess) {
        const firstErr = results.find((r) => r.status === "rejected") as
          ...
          | undefined
        throw new Error(
          firstErr?.reason?.message || "Both RE API requests failed"
        )
      }
      toast({
  ...
  const buildPayload = useCallback(() => {
    const payload: Record<string, unknown> = {
      ...
      flood_effective_date: formatDateOnly(
        floodEffective ?? DEFAULTS.floodEffective
      ),
      ...
      guarantor_borrower_ids: guarantorTags
        .filter((t) => t.id)
        .map((t) => t.id!),
  ...
  async function _logTermSheetActivity(
    ...
    try {
      ...
      if (scenarioIdOverride ?? selectedScenarioId) {
        formData.append(
          "scenarioId",
          scenarioIdOverride ?? selectedScenarioId ?? ""
        )
      }
      ...
      formData.append(
        "outputs",
        JSON.stringify(
          programResults?.map((r) => r.data ?? null).filter(Boolean) ?? null
        )
      )
      formData.append(
        "selected",
        JSON.stringify(selectedMainRow?.values ?? null)
      )
  ...
  async function handleCalculate() {
    try {
      ...
      if (!loanType) {
        toast({
          title: "Missing loan type",
          description: "Select a Loan Type before calculating.",
          variant: "destructive",
        })
        return
      }
      ...
      try {
        ...
        if (pre.ok) {
          const pj = (await pre.json().catch(() => ({}))) as {
            programs?: Array<{
              id?: string
              internal_name?: string
              external_name?: string
            ...
            try {
              if (!selfBrokerId) {
                ...
                const visRes = await fetch(
                  `/api/brokers/${encodeURIComponent(selfBrokerId)}/custom-settings`,
                  { cache: "no-store" }
                )
                const visJson = (await visRes.json().catch(() => ({}))) as {
                  program_visibility?: Record<string, boolean>
                }
                const visibility = (visJson?.program_visibility ??
                  {}) as Record<string, boolean>
                allowedIds = new Set<string>(
                  Object.keys(visibility).filter((k) => visibility[k] === true)
                )
                ph = ph.filter((p) => {
      ...
      try {
        ...
        void fetch(
          `https://n8n.axora.info/webhook/a108a42d-e071-4f84-a557-2cd72e440c83?_=${encodeURIComponent(nonce)}`,
          {
            method: "POST",
            cache: "no-store",
      ...
      await Promise.all(
        currentPlaceholders.map(async (p, idx) => {
          try {
            ...
            if (!memberIdLocal) {
              memberIdLocal = await waitForSelfMemberId().catch(
                () => null as any
              )
            }
            ...
            const res = await fetch(
              `/api/pricing/dispatch-one?_=${encodeURIComponent(nonce)}-${idx}`,
              {
                method: "POST",
                cache: "no-store",
                headers: {
                ...
                body: JSON.stringify({
                  loanType,
                  programId: p.id ?? p.internal_name ?? p.external_name,
                  data: {
                    ...payload,
                    organization_member_id: memberIdLocal ?? null,
                  },
                }),
              }
            )
            ...
            setProgramResults((prev) => {
              ...
              try {
                ...
                const arr = Array.isArray(dat?.initial_pitia)
                  ? (dat.initial_pitia as Array<
                      string | number | null | undefined
                    >)
                  : null
                const cache =
                  arr != null
                    ? Object.fromEntries(arr.map((v, i) => [i, v ?? null]))
                    : (next[idx] as any)?.initial_pitia_cache
                ...
                try {
                  ...
                  defaultOrig = pickDefault(
                    (dat as any)?.default_lender_orig_percent,
                    hi
                  ).trim()
                  defaultAdmin = pickDefault(
                    (dat as any)?.default_lender_admin_fee,
                    hi
                  ).trim()
                } catch {}
                const lenderDefaultsCache =
                  typeof (next as any)[idx]?.lender_defaults_cache === "object"
                    ...
                    : {}
                if (defaultOrig !== "")
                  lenderDefaultsCache.default_lender_orig_percent = defaultOrig
                if (defaultAdmin !== "")
                  lenderDefaultsCache.default_lender_admin_fee = defaultAdmin
                ;(next as any)[idx] = {
                  ...(next[idx] as any),
                  ...(single as any),
    ...
    try {
      ...
      if (!selected) {
        ...
        if (first) {
          ...
          if (isBridgeStyle) {
            const loanPrice =
              pick<string | number>(first.loan_price, hi) ?? null
            const rate = pick<string | number>(first.interest_rate, hi) ?? null
            const init =
              pick<string | number>(first.initial_loan_amount, hi) ?? null
            const hold = pick<string | number>(first.rehab_holdback, hi) ?? null
            const tot =
              pick<string | number>(first.total_loan_amount, hi) ?? null
            ...
            selected = {
              ...
              interestRate:
                pick<string | number>(first.interest_rate, hi) ?? null,
              loanAmount:
                (first.loan_amount as string | number | null | undefined) ??
                null,
      ...
      const selectedWithMeta = {
        ...
        program_id:
          ...
          programResults?.[selectedMainRow?.programIdx ?? 0]?.id ??
          null,
      ...
      toast({
        title: "Saved",
        description: `Scenario saved${j?.scenarioId ? ` (#${j.scenarioId})` : ""}.`,
      })
    } catch (err) {
      const message = err instanceof Error ? err.message : "Unknown error"
      toast({
  ...
  const rehabSectionVisible =
    isBridge && (bridgeType === "bridge-rehab" || bridgeType === "ground-up")
  ...
  function applyInputsPayload(payload: Record<string, unknown>) {
    ...
    if ("loan_type" in payload)
      setLoanType((payload["loan_type"] as string) ?? undefined)
    if ("transaction_type" in payload)
      setTransactionType((payload["transaction_type"] as string) ?? undefined)
    if ("property_type" in payload)
      setPropertyType((payload["property_type"] as string) ?? undefined)
    ...
    if ("purchase_price" in payload)
      setPurchasePrice(String(payload["purchase_price"] ?? ""))
    if ("loan_amount" in payload)
      setLoanAmount(String(payload["loan_amount"] ?? ""))
    ...
    if ("payoff_amount" in payload)
      setPayoffAmount(String(payload["payoff_amount"] ?? ""))
    if ("aiv" in payload) setAiv(String(payload["aiv"] ?? ""))
    if ("arv" in payload) setArv(String(payload["arv"] ?? ""))
    if ("rehab_budget" in payload)
      setRehabBudget(String(payload["rehab_budget"] ?? ""))
    if ("rehab_holdback" in payload)
      setRehabHoldback(String(payload["rehab_holdback"] ?? ""))
    if ("emd" in payload) setEmd(String(payload["emd"] ?? ""))
    if ("taxes_annual" in payload)
      setAnnualTaxes(String(payload["taxes_annual"] ?? ""))
    if ("hoi_annual" in payload)
      setAnnualHoi(String(payload["hoi_annual"] ?? ""))
    if ("flood_annual" in payload)
      setAnnualFlood(String(payload["flood_annual"] ?? ""))
    if ("hoa_annual" in payload)
      setAnnualHoa(String(payload["hoa_annual"] ?? ""))
    if ("hoi_premium" in payload)
      setHoiPremium(String(payload["hoi_premium"] ?? ""))
    if ("flood_premium" in payload)
      setFloodPremium(String(payload["flood_premium"] ?? ""))
    if ("mortgage_debt" in payload)
      setMortgageDebtValue(String(payload["mortgage_debt"] ?? ""))
    if ("tax_escrow_months" in payload)
      setTaxEscrowMonths(String(payload["tax_escrow_months"] ?? ""))
    // Units (leased/gross/market) from scenario inputs
    const unitsFromPayload = (payload["units"] ??
      payload["unit_data"]) as unknown
    ...
    if ("borrower_type" in payload)
      setBorrowerType((payload["borrower_type"] as string) ?? undefined)
    if ("citizenship" in payload)
      setCitizenship((payload["citizenship"] as string) ?? undefined)
    if ("fico" in payload) setFico(String(payload["fico"] ?? ""))
    if ("borrower_name" in payload)
      setBorrowerName(String(payload["borrower_name"] ?? ""))
    const borrowerEntityId =
      typeof payload["borrower_entity_id"] === "string"
        ? (payload["borrower_entity_id"] as string)
        : undefined
    setSelectedEntityId(borrowerEntityId ?? undefined)
    if (borrowerEntityId) {
      setHasSessionEntity(true)
    }
    const guarantorNames = Array.isArray(payload["guarantors"])
      ? (payload["guarantors"] as string[])
      : []
    const guarantorIds = Array.isArray(payload["guarantor_borrower_ids"])
      ? (payload["guarantor_borrower_ids"] as unknown[]).filter(
          (g): g is string => typeof g === "string" && g.length > 0
        )
      : []
    // Build guarantorTags array - names with optional IDs
    const loadedTags: Array<{ name: string; id?: string }> = guarantorNames.map(
      (name, idx) => ({
        name,
    ...
    if ("uw_exception" in payload)
      setUwException((payload["uw_exception"] as string) ?? undefined)
    if ("section8" in payload)
      setSection8((payload["section8"] as string) ?? undefined)
    ...
    if ("broker_orig_percent" in payload)
      setBrokerOrig(String(payload["broker_orig_percent"] ?? ""))
    if ("title_recording_fee" in payload)
      setTitleRecordingFee(String(payload["title_recording_fee"] ?? ""))
    if ("seller_concessions" in payload)
      setSellerConcessions(String(payload["seller_concessions"] ?? ""))
    ...
    if ("bridge_type" in payload)
      setBridgeType((payload["bridge_type"] as string) ?? undefined)
    if ("fthb" in payload) setFthb((payload["fthb"] as string) ?? undefined)
    if ("loan_structure_type" in payload)
      setLoanStructureType(
        (payload["loan_structure_type"] as string) ?? undefined
      )
    if ("ppp" in payload) setPpp((payload["ppp"] as string) ?? undefined)
    if ("str" in payload) setStrValue((payload["str"] as string) ?? undefined)
    if ("declining_market" in payload)
      setDecliningMarket((payload["declining_market"] as string) ?? undefined)
    if ("rentals_owned" in payload)
      setRentalsOwned(String(payload["rentals_owned"] ?? ""))
    if ("num_flips" in payload) setNumFlips(String(payload["num_flips"] ?? ""))
    if ("num_gunc" in payload) setNumGunc(String(payload["num_gunc"] ?? ""))
    if ("other_exp" in payload)
      setOtherExp((payload["other_exp"] as string) ?? undefined)
    if ("warrantability" in payload)
      setWarrantability((payload["warrantability"] as string) ?? undefined)
  }
    ...
    ;(async () => {
      try {
        ...
        setSelectedMainRow({
          ...
          rowIdx:
            (sel["row_index"] as number | undefined) ??
            (sel["rowIndex"] as number | undefined) ??
            0,
          programName:
            (sel["program_name"] as string | undefined) ??
            (sel["programName"] as string | undefined),
          programId:
            (sel["program_id"] as string | undefined) ??
            (sel["programId"] as string | undefined),
          values: isBridgeSel
            ? {
                loanPrice:
                  ((sel["loan_price"] ?? sel["loanPrice"]) as
                    | number
                    | string
                    | null) ?? null,
                interestRate: (sel["rate"] ?? sel["interestRate"]) as
                  | number
                  | string
                  | null,
                initialLoanAmount: (sel["initial_loan_amount"] ??
                  sel["initialLoanAmount"]) as number | string | null,
                rehabHoldback: (sel["rehab_holdback"] ??
                  sel["rehabHoldback"]) as number | string | null,
                loanAmount: (sel["total_loan_amount"] ?? sel["loanAmount"]) as
                  | number
                  | string
                  | null,
                pitia: (sel["funded_pitia"] ?? sel["pitia"]) as
                  | number
                  | string
                  | null,
                ltv: null,
            ...
            : {
                ...
                interestRate: (sel["rate"] ?? sel["interestRate"]) as
                  ...
                  | string
                  | null,
                loanAmount: (sel["loan_amount"] ?? sel["loanAmount"]) as
                  | number
                  | string
                  | null,
  ...
  function applyPlaceById(placeId: string) {
    ...
    svc.getDetails(req, (place, status: string) => {
      ...
      const get = (t: string) =>
        comps.find((c: AddressComponent) => c.types?.includes(t))
    ...
    >
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <div className="lg:hidden">
          <Tabs
            value={mobileView}
            onValueChange={(v) => setMobileView(v as "inputs" | "programs")}
          >
      ...
      >
          ...
          <div className="grid h-full min-h-0 max-h-full grid-rows-[auto_1fr_auto] rounded-md border overflow-hidden">
              ...
              <div className="flex min-w-0 flex-col gap-1">
                ...
                {isNamingScenario || isRenamingScenario ? (
                  <Input
                    ...
                    placeholder={
                      isRenamingScenario ? "Rename scenario" : "Scenario name"
                    }
                      ...
                      {scenariosList.map((s) => (
                        <SelectItem key={s.id} value={s.id}>
                          <div className="flex w-full items-center justify-between">
                            <span>
                              {s.name ??
                                `Scenario ${new Date(s.created_at ?? "").toLocaleDateString()}`}
                            </span>
                            {s.primary ? (
                              <IconStarFilled className="ml-2 h-3 w-3 text-yellow-500" />
                            ) : null}
                          </div>
                        </SelectItem>
                      ))}
                        ...
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const res = await fetch(
                                  `/api/scenarios/${selectedScenarioId}/primary`,
                                  { method: "POST" }
                                )
                                if (!res.ok) return
                                setScenariosList((prev) =>
                                  prev.map((s) => ({
                                    ...s,
                                    primary: s.id === selectedScenarioId,
                                  }))
                                )
                              } catch {
                                // ignore
                    ...
                    >
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            This will permanently remove the scenario from this
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction
                            onClick={async () => {
                              ...
                              try {
                                const res = await fetch(
                                  `/api/scenarios/${selectedScenarioId}`,
                                  { method: "DELETE" }
                                )
                                if (!res.ok) return
                                setScenariosList((prev) =>
                                ...
                                toast({
                                  title: "Delete failed",
                                  description: "Could not delete scenario.",
                                  variant: "destructive",
                                })
                              }
                            }}
            ...
            <div className="min-h-0 overflow-y-auto">
                ...
                >
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="delayed-purchase">
                                Delayed Purchase
                              </SelectItem>
                              <SelectItem value="rt-refi">
                                Refinance Rate/Term
                              </SelectItem>
                              <SelectItem value="co-refi">
                                Refinance Cash Out
                              </SelectItem>
                            </SelectContent>
                          </Select>
                                ...
                                <TooltipProvider>
                                  <Tooltip delayDuration={50}>
                                    <TooltipTrigger>
                                      <IconInfoCircle
                                        size={12}
                                        className="text-muted-foreground stroke-[1.25]"
                                      />
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <TooltipProvider>
                                  <Tooltip delayDuration={50}>
                                    ...
                                    <TooltipContent>
                                      Mortgage Debt shown on guarantor(s) credit
                                      report
                                    </TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <TooltipProvider>
                              <Tooltip delayDuration={50}>
                                ...
                                <TooltipContent>
                                  Middle score when 3 tradelines available, or
                                  lower score if only 2 are available
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </div>
                      ...
                      <AccordionContent>
                        <div className="grid gap-4 sm:grid-cols-2">
                          <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-1">
                              ...
                              <TooltipProvider>
                                <Tooltip delayDuration={50}>
                                  ...
                                  <TooltipContent>
                                    Properties owned (fix & holds should be
                                    included under '# of Flips')
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                            </div>
                            <NumberField
                              value={
                                rentalsOwned ? Number(rentalsOwned) : undefined
                              }
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <TooltipProvider>
                                <Tooltip delayDuration={50}>
                                  ...
                                  <TooltipContent>
                                    Flips exited in trailing 36 months
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                            </div>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <TooltipProvider>
                                <Tooltip delayDuration={50}>
                                  ...
                                  <TooltipContent>
                                    Ground Up projects exited in trailing 36
                                    months
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                              ...
                              {showPredictions && predictions.length > 0 && (
                                ...
                                >
                                  {predictions.map((p, idx) => {
                                    ...
                                    >
                                      <IconMapPin className="text-muted-foreground mt-0.5 h-4 w-4 shrink-0" />
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                          <div className="flex flex-col gap-1">
                            ...
                            >
                              <SelectTrigger
                                id="property-type"
                                className={`h-9 w-full ${reAuto.propertyType ? "border-2 border-amber-500/70" : ""}`}
                              >
                                <SelectValue placeholder="Select..." />
                              </SelectTrigger>
                              <SelectContent>
                                ...
                                <SelectItem value="pud">
                                  Townhome/PUD
                                </SelectItem>
                                <SelectItem value="condo">
                                  Condominium
                                </SelectItem>
                                <SelectItem value="mf2_4">
                                  Multifamily 2-4 Units
                                </SelectItem>
                                <SelectItem value="mf5_10">
                          ...
                          {propertyType === "condo" ? (
                            <div className="flex flex-col gap-1">
                              <Label htmlFor="warrantability">
                                Warrantability
                              </Label>
                              <Select
                                value={warrantability}
                                onValueChange={setWarrantability}
                              >
                                <SelectTrigger
                                  id="warrantability"
                                  className="h-9 w-full"
                                ...
                                <SelectContent>
                                  ...
                                  <SelectItem value="non-warrantable">
                                    Non-Warrantable
                                  </SelectItem>
                                </SelectContent>
                              </Select>
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            >
                              <SelectTrigger
                                id="num-units"
                                className={`h-9 w-full ${reAuto.numUnits ? "border-2 border-amber-500/70" : ""}`}
                              >
                                <SelectValue placeholder="Select..." />
                              </SelectTrigger>
                              <SelectContent>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <TooltipProvider>
                                <Tooltip delayDuration={50}>
                                  ...
                                  <TooltipContent>
                                    Gross Living Area Square Footage of subject
                                    property
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                          ...
                          {loanType === "dscr" && (
                            <>
                              ...
                              <div className="flex flex-col gap-1">
                                <Label htmlFor="declining-market">
                                  Declining Market
                                </Label>
                                <Select
                  ...
                  {loanType === "bridge" &&
                    (bridgeType === "bridge-rehab" ||
                      bridgeType === "ground-up") && (
                        ...
                        <AccordionContent>
                          <div className="grid gap-4 sm:grid-cols-2">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <TooltipProvider>
                                  <Tooltip delayDuration={50}>
                                    ...
                                    <TooltipContent>
                                      Rehab includes expanding the gross living
                                      area square footage by over 20% of current
                                    </TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
                              </div>
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <TooltipProvider>
                                  <Tooltip delayDuration={50}>
                                    ...
                                    <TooltipContent>
                                      Change of property use (ex. converting
                            ...
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center gap-1">
                                <Label htmlFor="arv">
                                  ARV <span className="text-red-600">*</span>
                                </Label>
                                <TooltipProvider>
                                  <Tooltip delayDuration={50}>
                                    <TooltipTrigger>
                  ...
                  <AccordionItem value="loan-structure" className="border-b">
                    ...
                    <AccordionContent>
                      <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        {loanType === "bridge" && (
                          <div className="flex flex-col gap-1">
                            ...
                            >
                              <SelectTrigger
                                id="term"
                                className={`h-9 w-full ${!touched.term && term === "12" ? "text-muted-foreground" : ""}`}
                              >
                        ...
                        <div className="flex flex-col gap-1">
                          <Label htmlFor="broker-admin-fee">
                            Broker Admin Fee
                          </Label>
                          <div className="relative">
                            <span className="text-muted-foreground pointer-events-none absolute top-1/2 left-2 -translate-y-1/2">
                  ...
                  >
                    ...
                    <AccordionContent>
                      <div className="grid gap-4 sm:grid-cols-2">
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="relative">
                            ...
                            <Input
                              id="borrower-name"
                              placeholder={
                                borrowerName || DEFAULTS.borrowerName
                              }
                              ...
                              className={`peer pr-9 pl-9 [&::-webkit-search-cancel-button]:appearance-none ${selectedEntityId && hasSessionEntity ? "border-blue-500 ring-1 ring-blue-500" : ""}`}
                              autoComplete="off"
                            />
                            {entityLoading && (
                                  ...
                                  <ul className="max-h-56 overflow-auto">
                                    {entitySuggestions.map((opt) => (
                                      <li key={opt.id}>
                                        <button
                                          type="button"
                                          className="hover:bg-muted w-full cursor-pointer rounded-sm px-2 py-1 text-left text-sm"
                                          onMouseDown={(e) => {
                          ...
                          <div className="relative">
                            <TagsInput
                              ...
                              onValueChange={(newValues) => {
                                ...
                                setGuarantorTags((prev) =>
                                  prev.filter((t) => newValues.includes(t.name))
                                )
                              }}
                              className="w-full"
                            >
                              <TagsInputList className="max-h-9 min-h-9 flex-nowrap overflow-x-auto overflow-y-hidden px-3 py-1">
                                <SearchIcon className="text-muted-foreground mr-1 size-4 shrink-0" />
                                {guarantorTags.map((tag, idx) => (
                                  <TagsInputItem
                                    key={`${tag.name}-${idx}`}
                                    value={tag.name}
                                    className={`shrink-0 px-1.5 py-0.5 text-xs ${tag.id ? "border-blue-500 ring-1 ring-blue-500" : ""}`}
                                  >
                                    {tag.name}
                                  </TagsInputItem>
                                ))}
                            ...
                            {showGuarantorSuggestions &&
                              guarantorSuggestions.filter(
                                (s) => !guarantorTags.some((t) => t.id === s.id)
                              ).length > 0 && (
                                        ...
                                        <li key={opt.id}>
                                          <button
                                            ...
                                            onMouseDown={(e) => {
                                              e.preventDefault() // Prevent blur from firing
                                              // Prevent exceeding 4 guarantor limit
                                              if (guarantorTags.length >= 4)
                                                return
                                              // Add as linked tag with borrower ID
                                              setGuarantorTags((prev) => [
                                                ...prev,
                                                { name: opt.name, id: opt.id },
                                              ])
                                              setGuarantorQuery("")
                                              setShowGuarantorSuggestions(false)
                                            }}
                        ...
                        <div className="flex flex-col gap-1">
                          <Label htmlFor="seller-concessions">
                            Seller Concessions
                          </Label>
                          <div className="relative">
                            <span className="text-muted-foreground pointer-events-none absolute top-1/2 left-2 -translate-y-1/2">
        ...
        <div className="relative hidden h-full items-stretch lg:flex">
          <button
            ...
            className={`hover:bg-accent flex h-full w-3 cursor-col-resize items-center justify-center ${isResizing ? "bg-accent" : ""}`}
            onMouseDown={(e) => {
              e.preventDefault()
              setIsResizing(true)
            }}
        ...
        >
          {resultsStale && !isDispatching ? (
            <div className="mb-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-100">
              Terms may be outdated based on recent input changes. Please
          ...
          <ResultsPanel
            ...
            onApplyFees={(lo, la) => {
              if (typeof lo === "string" && lo.trim().length > 0)
                setLenderOrig(lo)
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="text-muted-foreground text-sm">
                {fullAddress ||
                  "Enter street, city, state, and zip to preview the subject property."}
              </div>
                ...
                {mapsCenter && !mapsLoading ? (
                  mapsView === "street" ? (
                    <div className="relative h-full w-full">
                      ...
                      {!gmaps ||
                      !streetViewPosition ||
                      streetViewStatus !== gmaps.StreetViewStatus.OK ? (
                        <div className="text-muted-foreground absolute inset-0 flex items-center justify-center text-sm">
                          {streetViewStatus ===
                          gmaps?.StreetViewStatus?.ZERO_RESULTS
                  ...
                  <div className="text-muted-foreground flex h-full items-center justify-center text-sm">
                    {mapsLoading
                      ? "Locating address…"
                      : "Enter a complete address to preview."}
                  </div>
                )}
              </div>
            ) : null}
...
function ResultCard({
  ...
  const logCardTermSheetActivity = async (
    action: "downloaded" | "shared",
    pdfFile: File
  ) => {
    try {
      ...
      formData.append(
        "outputs",
        JSON.stringify([r.data ?? null].filter(Boolean))
      )
  ...
  const renderPreviewToPdf = async (): Promise<File | null> => {
    ...
    try {
      const disclaim = clone.querySelector(
        ".ts-disclaimer"
      ) as HTMLElement | null
      if (disclaim) {
      ...
      // Move logo down 5px (PDF-only)
      {
        ...
        if (headerEl) {
          const logoEl = headerEl.querySelector(
            'img[alt="Logo"]'
          ) as HTMLElement | null
          if (logoEl) {
      ...
      {
        const bridgeRoot = clone.querySelector(
          '[data-termsheet-root="bridge"]'
        ) as HTMLElement | null
        if (bridgeRoot) {
          // Find by text content to be robust against class changes
          const candidates = Array.from(
            bridgeRoot.querySelectorAll("div,span,p,h1,h2,h3")
          ) as HTMLElement[]
          const opt = candidates.find((el) =>
            (el.textContent || "").includes("{{ option_number }}")
          )
          if (opt) {
      ...
      if (debitsHeader) {
        const debitsPanel = debitsHeader.closest(
          ".border-2"
        ) as HTMLElement | null
        if (debitsPanel) {
          const list = debitsPanel.querySelector(
            ".space-y-1"
          ) as HTMLElement | null
          if (list) {
          ...
          const extraRows = debitsPanel.querySelectorAll(
          ...
          const pr2Blocks = Array.from(
            debitsPanel.querySelectorAll(".pr-2")
          ) as HTMLElement[]
          const emdBlock = pr2Blocks.length
            ? pr2Blocks[pr2Blocks.length - 1]
            : null
          if (emdBlock) {
            const cashOutRow = emdBlock.querySelector(
              ":scope > div:last-child"
            ) as HTMLElement | null
            if (cashOutRow) {
              const left = cashOutRow.querySelector(
                "span:first-child"
              ) as HTMLElement | null
              if (left) {
      ...
      if (liqHeader) {
        ...
        const liqList = liqContainer
          ? (liqContainer.querySelector(".space-y-1") as HTMLElement | null)
          : null
        if (liqList) {
    ...
    try {
      const bridgeRoot = container.querySelector(
        '[data-termsheet-root=\"bridge\"]'
      ) as HTMLElement | null
      if (bridgeRoot) {
        const leftBox = bridgeRoot.querySelector(
          "section.border-2.border-solid.border-black"
        ) as HTMLElement | null
        const rightCol = bridgeRoot.querySelector(
          "section.border-0"
        ) as HTMLElement | null
        if (leftBox && rightCol) {
          const rightRect = rightCol.getBoundingClientRect()
          const targetH =
            rightRect && rightRect.height ? Math.ceil(rightRect.height) : 0
          if (targetH > 0) {
  ...
  const programName = (
    isBroker
      ? (r?.external_name ?? "")
      : (r?.internal_name ?? r?.external_name ?? "")
  ) as string
  const isBridgeProgramName = String(programName)
    .toLowerCase()
    .includes("bridge")
  ...
  const pitia = isBridgeResp
    ? pick<string | number>(d?.funded_pitia, hi)
    : pick<string | number>(d?.pitia, hi)
  const dscr = isBridgeResp ? undefined : pick<string | number>(d?.dscr, hi)
  const loanAmount = isBridgeResp
    ? pick<string | number>(d?.total_loan_amount, hi)
    : d?.loan_amount
  const ltv = d?.ltv
  const TERMSHEET_WEBHOOK =
    "https://n8n.axora.info/webhook/a108a42d-e071-4f84-a557-2cd72e440c83"
  ...
  const rawWarn = Array.isArray(d.warning)
    ...
    : Array.isArray((d as Record<string, unknown>)?.warnings as unknown[])
      ? ((d as Record<string, unknown>).warnings as (
          | string
          | null
          | undefined
        )[])
  ...
  async function openTermSheetPreview(
    rowIndex?: number,
    opts?: { autoDownloadPdf?: boolean; autoShare?: boolean }
  ) {
    try {
      ...
      const rawInputs = (
        typeof getInputs === "function" ? getInputs() : {}
      ...
      if (isBridgeResp) {
        payloadRow["initial_loan_amount"] = pick<string | number>(
          d?.initial_loan_amount,
          idx
        )
        // include initial_pitia (from response or cache)
        const ip = pick<string | number>(d?.initial_pitia as any, idx)
        payloadRow["initial_pitia"] =
          ip ?? (r as any)?.initial_pitia_cache?.[idx]
        payloadRow["rehab_holdback"] = pick<string | number>(
          d?.rehab_holdback,
          idx
        )
        payloadRow["total_loan_amount"] = pick<string | number>(
          d?.total_loan_amount,
          idx
        )
      ...
      const body = {
        program: isBroker
          ? (r.external_name ?? "Program")
          : (r.internal_name ?? r.external_name ?? "Program"),
        ...
        inputs: (() => {
          ...
          const selLenderOrig = toStr(
            pickAt<any>((d as any)["lender_orig_percent"], idx)
          )
          const selLenderAdmin = toStr(
            pickAt<any>((d as any)["lender_admin_fee"], idx)
          )
      ...
      const nonce = `${Date.now()}-${Math.random().toString(36).slice(2)}`
      const res = await fetch(
        `${TERMSHEET_WEBHOOK}?_=${encodeURIComponent(nonce)}`,
        {
          method: "POST",
          cache: "no-store",
          headers: {
      ...
      const json = Array.isArray(raw)
        ? (raw[0] as DSCRTermSheetProps)
        : (raw as DSCRTermSheetProps)
      let enriched: DSCRTermSheetProps =
        json && typeof json === "object" && !Array.isArray(json)
          ? ({
              loan_type:
                isBridgeResp || isBridgeProgramName ? "bridge" : "dscr",
              ...json,
            } as DSCRTermSheetProps)
          : ({
              loan_type:
                isBridgeResp || isBridgeProgramName ? "bridge" : "dscr",
            } as DSCRTermSheetProps)
      ...
      try {
        const retDefOrig = String(
          (json as any)?.default_lender_orig_percent ?? ""
        ).trim()
        const retDefAdmin = String(
          (json as any)?.default_lender_admin_fee ?? ""
        ).trim()
        ;(r as any).lender_defaults_cache = {
          default_lender_orig_percent: retDefOrig,
          default_lender_admin_fee: retDefAdmin,
          ...(typeof (r as any).lender_defaults_cache === "object"
            ? (r as any).lender_defaults_cache
            : {}),
        }
      } catch {
      ...
      try {
        ...
        if (!currentLogo) {
          const resLogo = await fetch("/api/org/company-branding", {
            cache: "no-store",
          })
          const jLogo = (await resLogo.json().catch(() => ({}))) as {
            logo_url?: string
          }
          const logoUrl =
            typeof jLogo?.logo_url === "string" && jLogo.logo_url.length > 0
              ? jLogo.logo_url
              : ""
          if (logoUrl) {
      ...
      if (opts?.autoDownloadPdf || opts?.autoShare) {
        setTimeout(async () => {
          try {
              ...
              try {
                if (nav?.share && canShareFiles) {
                  await nav.share({
                    files: [file],
                    title: "Term Sheet",
                    text: "See attached term sheet PDF.",
                  })
                ...
                if (
                  ...
                  toast({
                    title: "PDF error",
                    description: (shareErr as any)?.message || "Share failed",
                    variant: "destructive",
                  })
                }
              }
            ...
            const message =
              e instanceof Error ? e.message : "Failed to create PDF"
            if (!/cancel/i.test(message)) {
              toast({
                title: "PDF not ready",
                description: "Preparing term sheet, try again in a moment.",
                variant: "default",
              })
            }
          }
      ...
      const message =
        err instanceof Error ? err.message : "Failed to load term sheet"
      toast({
  ...
  return (
    <div className="mb-3 rounded-md border p-3">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  aria-label="Share"
                  onClick={() =>
                    openTermSheetPreview(undefined, { autoShare: true })
                  }
                >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  aria-label="Download"
                  onClick={() =>
                    openTermSheetPreview(undefined, { autoDownloadPdf: true })
                  }
                >
        ...
        isBridgeResp ? (
          <div className="mt-3 grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            <Widget
              ...
              value={pick<string | number>(d?.initial_loan_amount, hi)}
            />
            <Widget
              label="Rehab Holdback"
              value={pick<string | number>(d?.rehab_holdback, hi)}
            />
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-center text-sm">
                  <thead className="border-b">
                    <tr>
                      <th className="w-8 py-1 pr-3 text-left"></th>
                      ...
                      <th className="w-14 py-1 pr-3 text-left"></th>
                    </tr>
                  </thead>
                  <tbody>
                          ...
                          >
                                  ...
                                  onSelect({
                                    ...
                                    values: {
                                      ...
                                      loanAmount: isBridgeResp
                                        ? Array.isArray(d?.total_loan_amount)
                                          ? d.total_loan_amount[i]
                                          : undefined
                                        : (loanAmount ?? undefined),
                                      initialLoanAmount: isBridgeResp
                                        ? Array.isArray(d?.initial_loan_amount)
                                          ? d.initial_loan_amount[i]
                                          : undefined
                                        : undefined,
                                      rehabHoldback: isBridgeResp
                                        ? Array.isArray(d?.rehab_holdback)
                                          ? d.rehab_holdback[i]
                                          : undefined
                                        : undefined,
                                      ltv: isBridgeResp
                                      ...
                                      pitia: isBridgeResp
                                        ? Array.isArray(d?.funded_pitia)
                                          ? d.funded_pitia[i]
                                          : undefined
                                        : Array.isArray(d?.pitia)
                                          ? d.pitia[i]
                                          : undefined,
                                      dscr: isBridgeResp
                                        ? undefined
                                        : Array.isArray(d?.dscr)
                                          ? d.dscr[i]
                                          : undefined,
                                    },
                                  })
                            ...
                            {isBridgeResp ? (
                              ...
                              <>
                                ...
                                <td className="py-1 pr-3 text-center">
                                  {Array.isArray(d?.pitia) ? d.pitia[i] : ""}
                                </td>
                                <td className="py-1 pr-3 text-center">
                                  {Array.isArray(d?.dscr) ? d.dscr[i] : ""}
                                </td>
                              </>
                            )}
                            <td className="py-1 pr-3 text-left">
                              <div className="flex items-center gap-1">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    openTermSheetPreview(i, { autoShare: true })
                                  }
                                >
                                  <IconShare3 className="h-4 w-4" />
                                </Button>
      ...
      <Dialog open={mcpOpen} onOpenChange={setMcpOpen}>
        <DialogContent className="max-h-[90vh] gap-2 overflow-hidden px-6 pt-4 pb-3 max-sm:max-h-[95dvh] max-sm:w-[calc(100vw-1rem)] max-sm:px-4 max-sm:pt-2 max-sm:pb-2 sm:max-w-[min(860px,calc(100vw-2rem))]">
          ...
          <button
            ...
            className="bg-background text-muted-foreground hover:bg-accent hover:text-foreground focus:ring-ring absolute top-2 right-24 inline-flex h-8 w-8 items-center justify-center rounded-md border shadow-sm transition-all hover:scale-110 focus:ring-2 focus:ring-offset-2 focus:outline-none active:scale-95 disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4"
            onClick={async () => {
              try {
                const file = await renderPreviewToPdf()
                if (!file) throw new Error("Could not render PDF")
                ...
                const message =
                  e instanceof Error ? e.message : "Unable to share"
                toast({
                  title: "Share failed",
                  description: message,
                  variant: "destructive",
          ...
          <button
            ...
            className="bg-background text-muted-foreground hover:bg-accent hover:text-foreground focus:ring-ring absolute top-2 right-14 inline-flex h-8 w-8 items-center justify-center rounded-md border shadow-sm transition-all hover:scale-110 focus:ring-2 focus:ring-offset-2 focus:outline-none active:scale-95 disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4"
            onClick={async () => {
              try {
                const file = await renderPreviewToPdf()
                if (!file) throw new Error("Could not render PDF")
...
function Widget({
  ...
  return (
    <div className="min-w-0 rounded-md border p-2">
      ...
      <div className="w-full truncate overflow-hidden text-left text-[13px] leading-5 font-semibold tracking-tight tabular-nums sm:text-sm">
        {value ?? ""}
      </div>
    </div>
  )
...
function ResultCardLoader({
  meta,
  isBroker,
}: {
  meta?: { internal_name?: string; external_name?: string }
  isBroker: boolean
}) {
  return (
    <div className="mb-3 rounded-md border p-3">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-sm font-bold">
            {isBroker
              ? (meta?.external_name ?? "Program")
              : (meta?.internal_name ?? meta?.external_name ?? "Program")}
          </div>
          {!isBroker ? (
        ...
        <div className="inline-flex items-center rounded-md border border-white bg-transparent px-2 py-0.5 text-xs font-semibold text-black dark:border-black dark:text-white">
          Generating
        </div>
      </div>
...
function ResultsPanel({
  ...
  useEffect(() => {
    if (!selected) return
    try {
      const d = (results?.[selected.programIdx]?.data ?? {}) as
        | ProgramResponseData
        | any
  ...
  const logPanelTermSheetActivity = async (
    action: "downloaded" | "shared",
    pdfFile: File
  ) => {
    try {
      ...
      formData.append(
        "outputs",
        JSON.stringify(
          results?.map((r) => r.data ?? null).filter(Boolean) ?? null
        )
      )
      formData.append("selected", JSON.stringify(selected?.values ?? null))
      formData.append("originalPdf", pdfFile)
    ...
    if (Array.isArray(results) && results.length > 0) {
      ...
      if (selectedFromProps.programId) {
        const idxById = results.findIndex(
          (r) =>
            r.id === selectedFromProps.programId ||
            r.internal_name === selectedFromProps.programId ||
            r.external_name === selectedFromProps.programId
        )
      ...
      const lpArr = Array.isArray(d.loan_price)
        ? (d.loan_price as Array<string | number>)
        : []
      const target = Number(
        String(selectedFromProps.values.loanPrice ?? "").replace(
          /[^0-9.-]/g,
          ""
        )
      )
  ...
  async function openMainTermSheetPreview(opts?: {
    autoDownloadPdf?: boolean
    autoShare?: boolean
  }) {
    try {
      if (!selected) return
      const d = (results?.[selected.programIdx]?.data ??
        {}) as ProgramResponseData
      const isBridgeResp =
        Array.isArray(d?.total_loan_amount) ||
        Array.isArray(d?.initial_loan_amount) ||
        Array.isArray(d?.funded_pitia)
      ...
      const loanAmount = isBridgeResp
        ? Array.isArray(d?.total_loan_amount)
          ? d.total_loan_amount[idx]
          : undefined
        : d?.loan_amount
      const ltv = d?.ltv
      const payloadRow: Record<string, unknown> = {
        loan_price: Array.isArray(d?.loan_price)
          ? d.loan_price[idx]
          : undefined,
        interest_rate: Array.isArray(d?.interest_rate)
          ? d.interest_rate[idx]
          : undefined,
      }
      if (isBridgeResp) {
        payloadRow["initial_loan_amount"] = Array.isArray(
          d?.initial_loan_amount
        )
          ? d.initial_loan_amount[idx]
          : undefined
        ...
        payloadRow["rehab_holdback"] = Array.isArray(d?.rehab_holdback)
          ? d.rehab_holdback[idx]
          : undefined
        payloadRow["total_loan_amount"] = Array.isArray(d?.total_loan_amount)
          ? d.total_loan_amount[idx]
          : undefined
        payloadRow["funded_pitia"] = Array.isArray(d?.funded_pitia)
          ? d.funded_pitia[idx]
          : undefined
      } else {
        payloadRow["loan_amount"] = loanAmount
      ...
      const defLenderOrig = toStr(
        pickAt<any>((d as any)["default_lender_orig_percent"], idx) ??
          (d as any)["default_lender_orig_percent"] ??
          (results?.[selected.programIdx] as any)?.lender_defaults_cache
            ?.default_lender_orig_percent
      )
      const defLenderAdmin = toStr(
        pickAt<any>((d as any)["default_lender_admin_fee"], idx) ??
          (d as any)["default_lender_admin_fee"] ??
          (results?.[selected.programIdx] as any)?.lender_defaults_cache
            ?.default_lender_admin_fee
      )
      if (selLenderOrig) {
      ...
      const normalizedRow = toYesNoDeepGlobal(payloadRow) as Record<
        string,
        unknown
      >
      ...
      const nonce = `${Date.now()}-${Math.random().toString(36).slice(2)}`
      const res = await fetch(
        `${TERMSHEET_WEBHOOK_MAIN}?_=${encodeURIComponent(nonce)}`,
        {
          method: "POST",
          cache: "no-store",
          headers: {
      ...
      const enriched =
        json && typeof json === "object" && !Array.isArray(json)
          ? ({
              loan_type: isBridgeResp ? "bridge" : "dscr",
              ...json,
            } as DSCRTermSheetProps)
          : ({
              loan_type: isBridgeResp ? "bridge" : "dscr",
      ...
      try {
        ...
        if (prog && typeof prog === "object") {
          prog.lender_defaults_cache = {
            default_lender_orig_percent: retDefOrig,
            default_lender_admin_fee: retDefAdmin,
            ...(typeof prog.lender_defaults_cache === "object"
              ? prog.lender_defaults_cache
              : {}),
          }
        }
  ...
  const orderedResults = React.useMemo(() => {
    ...
    const pickLoanAndRate = (
      r: ProgramResult
    ): { loan: number; rate: number } => {
      const d = (r?.data ?? {}) as ProgramResponseData
      const hi = Number(d?.highlight_display ?? 0)
      const isBridgeResp =
        Array.isArray(d?.total_loan_amount) ||
      ...
      const loan = isBridgeResp
        ? parseNum(
            pick<string | number>(
              d?.total_loan_amount as (string | number)[] | undefined,
              hi
            )
          )
      ...
      const rate = parseNum(
        pick<string | number>(
          d?.interest_rate as (string | number)[] | undefined,
          hi
        )
      )
      return { loan, rate }
    ...
    arr.sort((a, b) => {
      ...
      if (!Number.isNaN(loanA) && !Number.isNaN(loanB) && loanA !== loanB)
        return loanB - loanA
      if (!Number.isNaN(rateA) && !Number.isNaN(rateB) && rateA !== rateB)
        return rateA - rateB
      return 0
    })
    return arr
  ...
  const mainLoanPriceDisplay: string | number | null = React.useMemo(() => {
    ...
    if (!selected) return "-"
    const d = (results[selected.programIdx]?.data ??
      null) as ProgramResponseData | null
    if (!d) return "-"
    const ratesArr = Array.isArray(d.interest_rate)
      ? (d.interest_rate as Array<string | number>)
      : []
    const pricesArr = Array.isArray(d.loan_price)
      ? (d.loan_price as Array<string | number>)
      : []
  ...
  useEffect(() => {
    ...
    if (selected.programId) {
      const idxById = results.findIndex(
        (r) =>
          r.id === selected.programId ||
          r.internal_name === selected.programId ||
          r.external_name === selected.programId
      )
      if (idxById >= 0) progIdx = idxById
    }
    ...
    for (let i = 0; i < pricesArr.length; i++) {
      ...
      if (ratesArr.length > i) {
        ...
        if (Number.isFinite(targetRate)) {
          if (!Number.isFinite(r) || Math.abs(r - targetRate) >= tolRate)
            continue
        }
      }
      chosenIdx = i
    ...
    const nextValues = isBridgeResp
      ? {
          loanPrice:
            pick<string | number>(d?.loan_price as any, chosenIdx) ?? null,
          interestRate:
            pick<string | number>(d?.interest_rate as any, chosenIdx) ?? null,
          initialLoanAmount:
            pick<string | number>(d?.initial_loan_amount as any, chosenIdx) ??
            undefined,
          rehabHoldback:
            pick<string | number>(d?.rehab_holdback as any, chosenIdx) ??
            undefined,
          loanAmount:
            pick<string | number>(d?.total_loan_amount as any, chosenIdx) ??
            undefined,
          pitia:
            pick<string | number>(d?.funded_pitia as any, chosenIdx) ??
            undefined,
          ltv: undefined,
          dscr: undefined,
        }
  ...
  if (
    ...
    const filtered = selectedKey
      ? placeholders.filter(
          (p) =>
            p.id !== selectedKey &&
            p.internal_name !== selectedKey &&
            p.external_name !== selectedKey
        )
      : placeholders
    return (
      <div>
        {selected ? (
          <>
            <div className="bg-muted/40 mb-3 rounded-md border p-3">
                ...
                <div>
                  ...
                  <div className="text-muted-foreground text-xs font-semibold">
                    {(() => {
                      ...
                      const byPh = placeholders?.find?.(
                        (p) =>
                          p.id === selected.programId ||
                          p.internal_name === selected.programId ||
                          p.external_name === selected.programId
                      )
                      const name =
                        byResultsExt ??
                        byPh?.external_name ??
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          size="icon"
                          variant="ghost"
                          aria-label="Preview main"
                          onClick={() => openMainTermSheetPreview()}
                        >
                          <IconEye className="h-4 w-4" />
                        </Button>
                      </TooltipTrigger>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          onClick={() =>
                            openMainTermSheetPreview({ autoShare: true })
                          }
                        >
                          <IconShare3 className="h-4 w-4" />
                        </Button>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          onClick={() =>
                            openMainTermSheetPreview({ autoDownloadPdf: true })
                          }
                        >
                          <IconDownload className="h-4 w-4" />
                        </Button>
              ...
              {selected.values.rehabHoldback != null ||
              selected.values.initialLoanAmount != null ? (
                <div className="mt-3 grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
                  <Widget label="Loan Price" value={mainLoanPriceDisplay} />
                  <Widget
                    label="Interest Rate"
                    value={selected.values.interestRate}
                  />
                  <Widget
                    label="Initial Loan Amount"
                    value={selected.values.initialLoanAmount}
                  />
                  <Widget
                    label="Rehab Holdback"
                    value={selected.values.rehabHoldback}
                  />
                  <Widget
                    label="Total Loan Amount"
                    value={selected.values.loanAmount}
                  />
              ...
              <DialogContent className="max-h-[90vh] gap-2 overflow-hidden px-6 pt-4 pb-3 max-sm:max-h-[95dvh] max-sm:w-[calc(100vw-1rem)] max-sm:px-4 max-sm:pt-2 max-sm:pb-2 sm:max-w-[min(860px,calc(100vw-2rem))]">
                ...
                <div className="space-y-3">
                  {Object.keys(sheetPropsMain ?? {}).length ? (
                    <ScaledTermSheetPreview
                      sheetProps={sheetPropsMain as DSCRTermSheetProps}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-muted-foreground text-xs font-semibold">
                {(() => {
                  const byResultsExt =
                    results?.[selected.programIdx]?.external_name
                  const byPh = placeholders?.find?.(
                    (p) =>
                      p.id === selected.programId ||
                      p.internal_name === selected.programId ||
  ...
  return (
    <div>
      ...
      <Dialog open={mcpOpenMain} onOpenChange={setMcpOpenMain}>
        <DialogContent className="max-h-[90vh] gap-2 px-6 pt-4 pb-3 max-sm:max-h-[95dvh] max-sm:w-[calc(100vw-1rem)] max-sm:px-4 max-sm:pt-2 max-sm:pb-2 sm:max-w-[min(860px,calc(100vw-2rem))]">
          <DialogHeader className="mb-1">
            <DialogTitle className="text-base">Term Sheet</DialogTitle>
          </DialogHeader>
          <button
