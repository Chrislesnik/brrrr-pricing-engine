...
export function ApplicationPartyEditor({
  ...
  // Entity state
  const [entityId, setEntityId] = useState<string | null | undefined>(
  ...
  const [guarantors, setGuarantors] = useState<Guarantor[]>(() =>
    (initialGuarantors ?? []).map((g) => ({
      id: g.id ?? null,
      name: g.name ?? "",
      email: g.email ?? null,
    }))
  )
  const [editingGuarantorKey, setEditingGuarantorKey] = useState<string | null>(
    null
  ) // `${idx}`
  ...
  const [guarantorEmailStatus, setGuarantorEmailStatus] = useState<
    Record<string, "idle" | "loading" | "success" | "error">
  >({})
  const [signedEmails, setSignedEmails] = useState<Set<string>>(
    () => new Set(initialSignedEmails.map((e) => e.toLowerCase()))
  )
  const [sentEmails, setSentEmails] = useState<Set<string>>(
    () => new Set(initialSentEmails.map((e) => e.toLowerCase()))
  )
  const [_signingLoaded, setSigningLoaded] = useState(
    initialSignedEmails.length > 0 || initialSentEmails.length > 0
  )
  ...
  useEffect(() => {
    ...
    const q =
      entityInput && entityInput.trim().length > 0 ? entityInput.trim() : "*"
  ...
  useEffect(() => {
    ...
    const q =
      guarantorInput && guarantorInput.trim().length > 0
        ? guarantorInput.trim()
        : "*"
    const ctrl = new AbortController()
    ...
    ;(async () => {
      try {
        const res = await fetch(
          `/api/applicants/borrowers?q=${encodeURIComponent(q)}`,
          { cache: "no-store", signal: ctrl.signal }
        )
        if (!res.ok) return
        const j = (await res.json().catch(() => ({}))) as {
        ...
        const existingIds = new Set(
          guarantors.map((g) => g.id).filter(Boolean) as string[]
        )
        const opts =
          (j.borrowers ?? [])
            ...
            .map((b) => ({
              ...
              name: [b.first_name ?? "", b.last_name ?? ""]
                ...
                .trim(),
              display:
                `${[b.first_name ?? "", b.last_name ?? ""].filter(Boolean).join(" ").trim()} ${(b.email ?? "").trim()}`.trim(),
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch(
          `/api/applications/progress?loanId=${encodeURIComponent(loanId)}`,
          { cache: "no-store", signal: ctrl.signal }
        )
        if (!res.ok) return
        const j = (await res.json().catch(() => ({}))) as {
  ...
  const linkedEntityIcon = entityId ? (
      ...
      {showBorrowerEntity ? (
        <div className="flex flex-col gap-1">
          ...
          <div
            className={cn(
              "flex items-center",
              editingEntity ? "gap-2" : "gap-1"
            )}
          >
            <div
              className={cn(
                "relative flex flex-col gap-1",
                editingEntity ? "w-72 max-w-[320px]" : "w-auto"
              )}
            >
              {editingEntity ? (
                <>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span
                    ...
                    className="text-destructive cursor-pointer rounded px-1 py-0.5 text-[11px] font-semibold hover:underline"
            ...
            <Button
              ...
              disabled
              aria-label={
                entityId ? "Linked borrower entity" : "Unlinked borrower entity"
              }
            >
              {linkedEntityIcon}
            </Button>
        ...
        {guarantors.length > 0 ? (
          guarantors.map((g, idx) => {
            ...
            const isLinked = Boolean(
              currentId &&
              !(
                currentId.startsWith("guarantor-") ||
                currentId.startsWith("guarantor-name-")
              )
            )
            const nameChanged = isEditing
              ? guarantorInput.trim() !== currentName.trim()
              : false
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  idx > 0 && "border-border border-t pt-3"
                )}
                onClick={(e) => e.stopPropagation()}
              >
                ...
                <div className="flex items-center gap-2">
                            ...
                            onKeyDown={(e) => {
                              ...
                              if (e.key === "Enter") {
                                ...
                                setGuarantors((prev) => {
                                  const next = [...prev]
                                  const trimmedName =
                                    guarantorInput.trim() || next[idx]?.name
                                  const wasLinked =
                                    next[idx]?.id &&
                                    !(
                                      next[idx]?.id?.startsWith("guarantor-") ||
                                      next[idx]?.id?.startsWith(
                                        "guarantor-name-"
                                      )
                                    )
                                  const nameChangedNow =
                                    trimmedName !== (next[idx]?.name ?? "")
                                  next[idx] = {
                                    ...next[idx],
                                    id:
                                      wasLinked && nameChangedNow
                                        ? null
                                        : (next[idx]?.id ?? null),
                                    name: trimmedName,
                                  }
                          ...
                          onClick={(e) => {
                            ...
                            setGuarantors((prev) => {
                              ...
                              const wasLinked =
                                ...
                                !(
                                  next[idx]?.id?.startsWith("guarantor-") ||
                                  next[idx]?.id?.startsWith("guarantor-name-")
                                )
                              const nameChangedNow =
                                trimmedName !== (next[idx]?.name ?? "")
                              next[idx] = {
                  ...
                  {!isEditing &&
                    (() => {
                      const emailWasSent =
                        currentEmail &&
                        sentEmails.has(currentEmail.toLowerCase())
                      const isSigned =
                        currentEmail &&
                        signedEmails.has((currentEmail ?? "").toLowerCase())
                      return (
                        <Button
                          variant="ghost"
                          size="icon"
                          className={cn(
                            "h-8 w-8 border",
                            emailWasSent
                              ? "border-yellow-300 bg-yellow-50 text-yellow-600 hover:border-yellow-400 hover:text-yellow-700"
                              : "border-muted text-muted-foreground hover:border-green-400 hover:text-green-600",
                            {
                              "pointer-events-none opacity-50":
                                !currentEmail || isSigned,
                            },
                            status === "error" &&
                              "animate-[email-shake_0.45s_ease-in-out] motion-reduce:animate-none"
                          )}
                          aria-label={
                            !currentEmail
                          ...
                          onClick={async (e) => {
                            ...
                            e.stopPropagation()
                            setGuarantorEmailStatus((prev) => ({
                              ...prev,
                              [statusKey]: "loading",
                            }))
                            try {
                              const res = await fetch(
                                "https://n8n.axora.info/webhook/3075cb28-a5db-499e-b916-95c7ce002dec",
                                {
                                  method: "POST",
                                  headers: {
                              ...
                              const j = await res
                                .json()
                                .catch(() => ({}) as { sent?: boolean })
                              const sent = Boolean((j as any)?.sent)
                              setGuarantorEmailStatus((prev) => ({
                                ...prev,
                                [statusKey]: sent ? "success" : "error",
                              }))
                        ...
                        >
                          {(() => {
                            ...
                            if (status === "success") {
                              return (
                                <Check
                                  className="animate-in fade-in zoom-in h-4 w-4 text-green-600"
                                  aria-hidden="true"
                                />
                              )
                            }
                  ...
                  {!isEditing && (
                    <div className="flex items-center gap-2">
                      ...
                      signedEmails.has((currentEmail ?? "").toLowerCase()) ? (
                        <Badge
                          variant="secondary"
                          className="h-6 border-green-200 bg-green-100 text-[11px] font-semibold text-green-700"
                        >
                          Signed
                        </Badge>
                      ) : null}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to remove this guarantor? This guarantor
              already submitted the signed application.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
