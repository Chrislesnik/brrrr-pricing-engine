...
import { authClient, useSession } from "@/components/workflow-builder/lib/auth-client";
import { integrationsAtom } from "@/components/workflow-builder/lib/integrations-store";
...
function ToolbarActions({
  ...
  const handleAddStep = () => {
    // Ensure a trigger node exists before adding action nodes
    const hasTrigger = state.nodes.some((n) => n.data.type === "trigger");
    if (!hasTrigger) {
      toast.error("Add a trigger node first before adding action steps.");
      return;
    }
