...
// ─────────────────────────────────────────────────────────────────────────────
// CurrencyCalcCell - Currency input with formula support (=expressions)
// ─────────────────────────────────────────────────────────────────────────────
...
function formatCurrencyWithCommas(raw: string): string {
  if (!raw) return "";
  const [i, d] = raw.split(".");
...
export function CurrencyCalcCell<TData>({
  ...
  const initialValue = cell.getValue() as string;
  const [value, setValue] = React.useState(initialValue ?? "");
  const [calcMode, setCalcMode] = React.useState(false);
  ...
  const onBlur = React.useCallback(() => {
    ...
    tableMeta?.onCellEditingStop?.();
  }, [tableMeta, rowIndex, columnId, initialValue, value, readOnly, calcMode, resolveExpression]);
  ...
  const onWrapperKeyDown = React.useCallback(
    ...
    [isEditing, isFocused, initialValue, tableMeta, rowIndex, columnId, value, calcMode, resolveExpression],
  );
  ...
  const displayValue = calcMode ? `=${expr}` : formatCurrencyWithCommas(value);
  ...
  return (
    <DataGridCellWrapper<TData>
      ...
      onKeyDown={onWrapperKeyDown}
      className={cn(
        calcMode && "ring-2 ring-purple-500/60 border-purple-500/70",
      )}
    >
      {isEditing ? (
