...
import { Send, Loader2, Sparkles, FileText, MessageSquare, ClipboardList, Check, X } from "lucide-react"
...
function DetailItem({
  ...
  const [status, setStatus] = React.useState<"neutral" | "approved" | "rejected">("neutral")
  ...
  return (
    <div
      className={cn(
        ...
        status === "approved" && "bg-green-50 border-green-300 dark:bg-green-950/30 dark:border-green-800",
        status === "rejected" && "bg-red-50 border-red-300 dark:bg-red-950/30 dark:border-red-800",
        status === "neutral" && "bg-card"
      )}
    >
      {/* Label */}
      <label className="text-sm font-semibold leading-none">{item.label}</label>

      {/* Input with check/x buttons */}
      <div className="flex h-9 w-full items-center overflow-hidden rounded-md border border-input bg-background shadow-xs">
        <Input
          value={value}
          onChange={(e) => {
          ...
          className="h-full flex-1 border-0 shadow-none focus-visible:ring-0 text-sm bg-transparent"
        />
        <button
          type="button"
          onClick={handleApprove}
          className={cn(
            "flex h-full aspect-square items-center justify-center border-l border-input transition-colors",
            status === "approved"
        ...
        <button
          ...
          className={cn(
            "flex h-full aspect-square items-center justify-center border-l border-input rounded-r-md transition-colors",
            status === "rejected"
              ...
              <span>
                ...
                {citation.snippet && (
                  <span className="text-muted-foreground ml-1">
                    — "{citation.snippet.substring(0, 30)}{citation.snippet.length > 30 ? "..." : ""}"
                  </span>
                )}
              </span>
            </button>
