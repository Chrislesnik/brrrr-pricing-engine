...
export function EntitiesTable({ data, initialOwnersMap }: Props) {
  ...
  const [entityDetailsLoading, setEntityDetailsLoading] = useState<Record<string, boolean>>({})
  ...
  const fetchOwners = async (entityId: string) => {
    ...
    try {
      const res = await fetch(`/api/applicants/entities/${encodeURIComponent(entityId)}/owners`, { cache: "no-store" })
      if (!res.ok) {
        setOwnersMap((prev) => ({ ...prev, [entityId]: null }))
        return
      }
      const j = (await res.json().catch(() => ({}))) as { owners?: EntityOwner[] }
      setOwnersMap((prev) => ({ ...prev, [entityId]: Array.isArray(j.owners) ? j.owners : [] }))
    } catch {
      setOwnersMap((prev) => ({ ...prev, [entityId]: null }))
    } finally {
  ...
  const fetchEntityDetails = async (entityId: string) => {
    if (entityDetailsMap[entityId] !== undefined || entityDetailsLoading[entityId]) return
    ...
    try {
      ...
      const j = (await res.json().catch(() => ({}))) as { entity?: EntityProfile }
      setEntityDetailsMap((prev) => ({ ...prev, [entityId]: (j?.entity as EntityProfile) ?? null }))
    } catch {
      setEntityDetailsMap((prev) => ({ ...prev, [entityId]: null }))
    } finally {
  ...
  const isAnyDialogOpen = () =>
    typeof document !== "undefined" && document.querySelector("[data-slot='dialog-content'][data-state='open']")
    ...
    return [
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            ...
            >
              <ChevronDown className={cn("h-4 w-4 transition-transform", isOpen ? "rotate-180" : "-rotate-90")} aria-hidden="true" />
            </Button>
          )
        },
        enableSorting: false,
      ...
      },
      {
        ...
        cell: ({ row }) => <span className="text-sm text-muted-foreground">{row.getValue("display_id") || "-"}</span>,
      ...
      },
      {
        ...
        cell: ({ row }) => <span className="text-sm font-semibold text-foreground">{row.getValue("entity_name") || "-"}</span>,
      },
      {
        header: "Entity Type",
        accessorKey: "entity_type",
        cell: ({ row }) => <span className="text-muted-foreground text-sm">{row.getValue("entity_type") || "-"}</span>,
      },
      {
        header: "EIN",
        accessorKey: "ein",
        cell: ({ row }) => <span className="text-muted-foreground text-sm">{formatEIN(row.original.ein)}</span>,
      },
      {
        header: "Date Formed",
        accessorKey: "date_formed",
        cell: ({ row }) => <span className="text-muted-foreground text-sm">{formatDate(row.original.date_formed)}</span>,
      },
      {
        header: "Assigned To",
        accessorKey: "assigned_to_names",
        cell: ({ row }) => {
          const names = Array.isArray(row.original.assigned_to_names) ? row.original.assigned_to_names : []
          return (
            <span className="text-muted-foreground text-sm">{names.length ? names.join(", ") : "Unassigned"}</span>
          )
        },
      },
      {
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => {
                ...
                if (isAnyDialogOpen()) return
                    ...
                    {isOpen ? (
                      ownersLoading[row.original.id] && ownersMap[row.original.id] === undefined ? (
                        <TableRow className="bg-muted/30">
                          <TableCell colSpan={columns.length} className="p-4 text-sm text-muted-foreground">
                            Loading members...
                          </TableCell>
                        </TableRow>
                      ) : ownersMap[row.original.id] == null ? (
                        <TableRow className="bg-muted/30">
                          <TableCell colSpan={columns.length} className="p-4 text-sm text-destructive">
          ...
          <>
            <TableRow className="bg-muted/30">
              <TableCell colSpan={columns.length} className="p-0">
                <div className="grid grid-cols-5 gap-4 px-6 py-2 text-[11px] font-semibold uppercase text-muted-foreground">
                  <span>ID</span>
                  <span>Name</span>
                  <span>Type</span>
                  <span>Title</span>
...
function InfoBlock({ label, value, bold = false }: { label: string; value: ReactNode; bold?: boolean }) {
  return (
    <div>
      <div className="text-[11px] font-semibold uppercase text-muted-foreground">{label}</div>
      <div className={cn("text-foreground", bold && "font-semibold")}>{value}</div>
    </div>
  )
}
...
function MemberRow({
  ...
  const displayId =
    (linkedId ? entity?.display_id ?? owner.entity_display_id : owner.borrower_display_id ?? owner.entity_display_id) ?? "—"
  const displayName = linkedId ? entity?.entity_name ?? owner.entity_display_name ?? owner.name ?? "-" : owner.name ?? "-"
  const displayType = linkedId ? entity?.entity_type ?? owner.member_type ?? "-" : owner.member_type ?? "-"
  const displayEin = linkedId ? formatEIN(entity?.ein) : "—"
  const displayDate = linkedId ? formatDate(entity?.date_formed) : "—"
  const displayAssigned =
    linkedId && Array.isArray(entity?.assigned_to_names) && entity.assigned_to_names.length
      ? entity.assigned_to_names.join(", ")
      : "—"
  const displayTitle = owner.title || "-"
  const displayOwnership = owner.ownership_percent != null ? `${owner.ownership_percent}%` : "-"

  const childOwners = linkedId ? ownersMap[linkedId] : undefined
  const childOwnersLoading = linkedId ? ownersLoading[linkedId] : false
  const entityLoading = linkedId ? entityDetailsLoading[linkedId] : false
    ...
    <>
      <TableRow className={cn("bg-muted/30", canExpand && "cursor-pointer")}>
        <TableCell colSpan={columns} className="p-0">
          <div
            className={cn("grid grid-cols-5 items-center gap-4 px-6 py-2 text-sm", canExpand && "hover:bg-muted/40")}
            onClick={
              canExpand
                ? (e) => {
                    e.stopPropagation()
            ...
            <div className="flex items-center" style={{ paddingLeft: depth * 16 }}>
              {canExpand ? (
                <Button
                  ...
                  aria-label={isExpanded ? "Collapse member entity" : "Expand member entity"}
                >
                  <ChevronDown
                    className={cn("h-4 w-4 transition-transform", isExpanded ? "rotate-180" : "-rotate-90")}
                    aria-hidden="true"
                  />
                </Button>
              ) : (
      ...
      {canExpand && isExpanded ? (
        childOwnersLoading && childOwners === undefined ? (
          <TableRow className="bg-muted/40">
            <TableCell colSpan={columns} className="p-4 text-sm text-muted-foreground">
              Loading members...
            </TableCell>
          </TableRow>
        ) : childOwners == null ? (
          <TableRow className="bg-muted/40">
            <TableCell colSpan={columns} className="p-4 text-sm text-destructive">
              Failed to load members.
            </TableCell>
          </TableRow>
